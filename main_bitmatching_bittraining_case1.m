...
%% LS data
LS_path = 'Local Structure\FVC2002\Db2_a\PCA50';
LS_list = dir(fullfile(LS_path, '*.mat'));
N_LS = length(LS_list);
%% Cluster load
% load('Center/FVC2004DB1_C4500_PCA100_8040_3f.mat');
load('Center/FVC2002DB2_C15000_PCA50_8040_case1.mat');

%% Bit-training load
% load('Bit_training/FVC2004DB1_C4500_PCA100_8040_3f.mat');
load('Bit_training/FVC2002DB2_C15000_PCA50_8040_case1.mat');

%% Save result
% save_path = 'Result\result_FVC2004DB1_4500_1selec_case2_bittraining.mat';
save_path = 'Result\result_FVC2002DB2_15000_5selec_case1_bittraining.mat';
...
N_select = 5; %Number of selected cluster for each minutia
...
for i=1:N_LS
    ...
    [bit, out_score, min_analysis] = f_conversion_bit_case1(structure, out_Cluster, N_select, mode);  
...
for i=1:N_sub
    ...
    Selected_bit_R(i,:) = f_result_bit_train_case1(train_bit, Discrim_score, num_minutiae, N_train, N_select);
    
    fprintf('\n');
end

for i=1:N_sub
    total_bit_T(1,:,i) = Selected_bit_R(i,:);
    for j=4:8
        total_bit_T(j-2,:,i) = total_bit((i-1)*8+j).bit;
    end
end
